...
const (
	...
	// Per API docs, the XML CORS document is limited to 64 KB in size
	// TODO: allow a bit of headroom here?
	maxBodyBytes = 64 * humanize.KiByte
)
...
func (api objectAPIHandlers) PutBucketCorsHandler(w http.ResponseWriter, r *http.Request) {
	...
	bucketCorsBytes, err := io.ReadAll(ioutil.HardLimitReader(r.Body, maxBodyBytes))
	if err != nil {
		writeErrorResponse(ctx, w, toAPIError(ctx, err), r.URL)
		return
	}
	...
	replLogIf(ctx, globalSiteReplicationSys.BucketMetaHook(ctx, madmin.SRBucketMeta{
		Type:      madmin.SRBucketMetaTypeCors,
		Bucket:    bucket,
		Cors:      &cfgStr,
		UpdatedAt: updatedAt,
	}))
